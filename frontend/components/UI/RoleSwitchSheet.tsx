...
export default function RoleSwitchSheet({
  ...
  return (
    <BottomSheet visible={visible} onClose={onClose}>
      <Text fontWeight="black" fontSize={20} classN={`text-[${twColor(colors.foreground)}]`}>
        Switch to {ROLE_LABEL[nextRole]}?
      </Text>
      <Text fontSize={13} classN={`mt-2 text-[${twColor(colors.mutedForeground)}]`}>
      ...
      <View style={tw`mt-5 gap-3`}>
        ...
        <Pressable
          ...
          style={[
            tw`items-center justify-center rounded-full py-4`,
            { borderWidth: 1, borderColor: colors.grey4 },
          ]}>
          <Text fontWeight="bold" fontSize={15} classN={`text-[${twColor(colors.foreground)}]`}>
            Cancel
          </Text>
        </Pressable>
      </View>
